...
def run_adb_command(adb_path, command):
    ...
    try:
        ...
        logger.error(f"ADB command failed: {' '.join(command)}")
...
def check_device_connection(adb_path):
    ...
    state = run_adb_command(adb_path, ["get-state"])
    if state == "device":
...
def install_certificate(adb_path, cert_file, cert_hash):
    ...
    if not run_adb_command(adb_path, ["root"]):
        logger.error("Failed to get root access")
        return False
    ...
    if not run_adb_command(adb_path, ["remount"]):
    ...
    if not run_adb_command(adb_path, ["push", cert_file, remote_path]):
    ...
    if not run_adb_command(adb_path, ["shell", f"chmod 644 {remote_path}"]):
